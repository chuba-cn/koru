...
| `branch` | Create/read/update under `/churches/:churchId/branches` | Tenant; every mutation (`POST`/`PATCH`) needs `super_admin`/`regional_admin`/`branch_admin` — `finance`/`recorder` read only; `GET` is [cursor-paginated](#paginated-lists-are-cursor-based-not-offset-based) and scope-narrowed the same way as `region`; mutations are authority-checked the same way, including both sides of a move between regions; a move is also refused if it would leave a branch-scoped campaign settling into a region-scoped account that no longer covers it — see [ADR-0021](../apps/api/docs/adr/0021-campaign-settlement-routing-and-repoint-lockout.md) |
...
value — [#140](https://github.com/chuba-cn/koru/issues/140)) is the wrong shape for a resource
