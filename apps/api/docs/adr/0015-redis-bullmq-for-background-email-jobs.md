...
Outbound email (chuba-cn/koru#64) is sent from a durable job queue, not inline in the request path
...
staff-facing resend action (chuba-cn/koru#68) is the recovery path.

This reopens a call made in chuba-cn/koru#64's first draft — "no queue, sends stay inline,
...
pre-launch and at this volume, and tracked as its own ticket (chuba-cn/koru#76) rather than solved
...
(chuba-cn/koru#73) is deliberately the only queue for now; a future job type (e.g. a Nudge-epic SMS
